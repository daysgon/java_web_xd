...
public class OrderDao {
    ...
    public int add(Order order){
        return JDBCUtil.executeUpdate("insert into orders(purchaser_id,good_id,good_name,seller_id,seller_name,price,amount,create_time,address) values(?,?,?,?,?,?,?,now(),?)"
                ,order.getPurchaserId(),order.getGoodId(),order.getGoodName(),order.getSellerId(),order.getSellerName(),order.getPrice(),order.getAmount(),order.getAddress());
    }
    ...
    public Order getOrderById(int id) {
        return JDBCUtil.QueryOne("select * from orders where order_id = ?", new RowMap<Order>() {
            @Override
            public Order rowMapping(ResultSet rs) {
                Order order = new Order();
                try {
